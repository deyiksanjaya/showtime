let startTime = 0;
        ...
        // === FLAG DAN STATE WORLD CLOCK BARU ===
        let worldClockReady = false; // Flag to indicate if World Clock mode is ready to be triggered by display tap
        let worldClockTriggered = false; // Flag to indicate if the 5-second delay has started
        let worldClockTarget = null; // Stores the target number for World Clock mode (same as psychicTarget)
        ...
        worldClockTab.addEventListener("click", () => {
            ...
            if (!running) {
                ...
                // Reset worldClockReady when opening the psychic overlay
                worldClockReady = false;
                worldClockTarget = null;
                worldClockTriggered = false; // Ensure this is reset too
            } else {
                    ...
                    if (currentValue.length === 2) {
                        setTimeout(() => {
                            ...
                            if (psychicTarget !== null && !isNaN(psychicTarget)) {
                                ...
                                // --- FIX: Set worldClockReady and worldClockTarget here ---
                                worldClockReady = true;
                                worldClockTarget = psychicTarget;
                                console.log("Psychic Target set:", psychicTarget, "World Clock Ready:", worldClockReady);
                            } else {
                                if (psychicIndicator) {
                                    psychicIndicator.classList.remove("active");
                                }
                                // --- FIX: Reset worldClockReady and worldClockTarget if target is invalid ---
                                worldClockReady = false;
                                worldClockTarget = null;
        ...
        clearBtn.addEventListener("click", () => {
            psychicInput.textContent = "--";
            // --- FIX: Reset worldClockReady and worldClockTarget when cleared ---
            worldClockReady = false;
            worldClockTarget = null;
            worldClockTriggered = false; // Reset triggered state
            if (psychicIndicator) {
                psychicIndicator.classList.remove("active");
            }
            psychicTarget = null; // Also clear psychicTarget
        });
        ...
        startBtn.onclick = () => {
            ...
            if (!running) { // START the stopwatch
                ...
                if (psychicTarget !== null && !isNaN(psychicTarget)) {
                    psychicTimeout = setTimeout(() => {
                        if (running) { // Ensure stopwatch is still running when timeout fires
                            ...
                            // --- FIX: Reset worldClockReady and worldClockTarget after psychic sequence completes ---
                            worldClockReady = false;
                            worldClockTarget = null;
                            worldClockTriggered = false; // Reset triggered state
                        }
                ...
                if (psychicTimeout) {
                    ...
                    // --- FIX: Reset worldClockReady and worldClockTarget when stopped manually ---
                    worldClockReady = false;
                    worldClockTarget = null;
                    worldClockTriggered = false; // Reset triggered state
                }
        ...
        lapBtn.onclick = () => {
            if (!running) { // RESET the stopwatch
                ...
                // --- FIX: Reset worldClockReady and worldClockTarget on full reset ---
                worldClockReady = false;
                worldClockTarget = null;
                worldClockTriggered = false; // Reset triggered state
            } else { // RECORD a LAP
            ...
            tab.addEventListener("click", () => {
                ...
                if (tab.id === "worldClockTab") {
                    if (!running) { // Only allow opening psychic mode if stopwatch is not running
                        ...
                        psychicInput.textContent = "--";
                        psychicInput.classList.add("active");
                        // Reset worldClockReady when opening the psychic overlay directly from tab
                        worldClockReady = false;
                        worldClockTarget = null;
                        worldClockTriggered = false;
                    }
        ...
        // === TAP DETECTION UNTUK TRIGGER WORLD CLOCK MODE ===

        console.log("✅ Event listener untuk display terpasang");

        stopwatchEl.addEventListener("click", () => {
            console.log("Display tapped. worldClockReady:", worldClockReady, "worldClockTriggered:", worldClockTriggered, "running:", running);
            if (worldClockReady && !worldClockTriggered && !running) {
                console.log("⏳ Tap display! Mulai countdown World Clock.");
                worldClockTriggered = true;
                triggerWorldClockSequence();
            } else if (running) {
                console.log("Stopwatch sedang berjalan, tidak bisa memicu World Clock.");
            } else if (worldClockTriggered) {
                console.log("World Clock sudah dipicu, menunggu countdown selesai.");
            } else if (!worldClockReady) {
                console.log("World Clock belum siap (target belum dimasukkan atau mode tidak aktif).");
            }
        });

        // === FUNGSI UTAMA WORLD CLOCK MODE ===
        function triggerWorldClockSequence() {
            // Ubah label ke "World Clock." setelah 4 detik
            setTimeout(() => {
                const label = document.querySelector("#worldClockTab div");
                if (label) label.textContent = "World Clock.";
                console.log("⌛ Label berubah jadi 'World Clock.'");
            }, 4000);

            // Setelah 5 detik, mulai stopwatch otomatis & ganti label kembali
            setTimeout(() => {
                const label = document.querySelector("#worldClockTab div");
                if (label) label.textContent = "World Clock";
                ...
                if (!running) {
                    ...
                    // Use worldClockTarget here, which is now aligned with psychicTarget
                    if (worldClockTarget !== null && !isNaN(worldClockTarget)) {
                        console.log("▶️ Stopwatch auto-mulai, target:", worldClockTarget);
                        psychicTimeout = setTimeout(() => {
                            if (running) {
                                running = false;
                                const now = performance.now();
                                ...
                                const secondsPart = Math.floor(elapsed / 1000) * 1000;
                                const adjustedMs = secondsPart + (worldClockTarget * 10); // Use worldClockTarget
                                elapsed = adjustedMs;
                                ...
                                psychicTarget = null; // Clear psychicTarget as well
                                worldClockReady = false;
                                worldClockTarget = null;
                                worldClockTriggered = false;

                                console.log("⏹️ Auto-stop & disesuaikan ke target.");
                            }
                        }, 6000);
                    }
                }
