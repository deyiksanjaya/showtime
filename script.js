...
const LICENSE_STORAGE_KEY = 'showtime_license_activated';
const REDIRECT_URL = 'index.html';

function initializeStopwatchApplication() {
  // --- Variabel dan State Aplikasi (Kode asli Anda dimulai di sini) ---
  let startTime = 0;
  let elapsed = 0;
  let running = false;
  let laps = [];
  ...
  // --- Elemen DOM (Pencarian elemen sekarang aman karena fungsi ini dipanggil setelah DOM siap) ---
  ...
  function getNextUniqueSumOfNineFromPool() {
    ...
    return shuffledSumOfNineMillisPool.pop();
  }
  ...
  // --- Logika Aplikasi dan Event Listeners ---
  ...
  alarmsTab.addEventListener("click", () => {
    ...
    if (mindReadingIndicator) {
      mindReadingIndicator.classList.toggle("active", mindReadingMode);
    }
    console.log("Mind Reading Mode: " + (mindReadingMode ? "Activated" : "Deactivated"));
    if (mindReadingMode) {
      initializeSumOfNinePool();
      if (!running) {
        elapsed = 0;
  ...
  // Kode ini sekarang berada di akhir fungsi, memastikan semua event listener terpasang.
  stopwatchTab.classList.add("active");
  initializeSumOfNinePool();
}

async function verifyAndStart() {
  // Cek 1: Apakah lisensi sudah diaktivasi di localStorage?
  if (localStorage.getItem(LICENSE_STORAGE_KEY) === 'true') {
    console.log('Lisensi sudah aktif. Menjalankan aplikasi...');
    initializeStopwatchApplication(); // Langsung jalankan aplikasi
    return;
  }

  // Cek 2: Apakah ada kunci lisensi di URL?
  const urlParams = new URLSearchParams(window.location.search);
  const licenseKeyFromUrl = urlParams.get('license_key');

  if (!licenseKeyFromUrl) {
    console.log('Kunci lisensi tidak ditemukan. Mengalihkan...');
    window.location.replace(REDIRECT_URL);
    return;
  }

  // Cek 3: Validasi kunci dengan Firebase
  try {
    ...
    if (snapshot.exists()) {
      ...
      const firebaseKey = Object.keys(licenses).find(key => licenses[key] === licenseKeyFromUrl.trim());

      if (firebaseKey) {
        // Kunci valid!
        console.log('Kunci lisensi valid. Mengaktifkan dan menjalankan aplikasi...');
        localStorage.setItem(LICENSE_STORAGE_KEY, 'true');
        const licenseToRemoveRef = ref(db, `data/licenses/${firebaseKey}`);
        await remove(licenseToRemoveRef);
        window.history.replaceState({}, document.title, window.location.pathname);
        initializeStopwatchApplication(); // Jalankan aplikasi setelah aktivasi
      } else {
        // Kunci tidak valid
        console.log('Kunci lisensi tidak valid. Mengalihkan...');
        window.location.replace(REDIRECT_URL);
      }
    } else {
      // Node lisensi tidak ada di DB
      console.log('Data lisensi tidak ditemukan di Firebase. Mengalihkan...');
      window.location.replace(REDIRECT_URL);
    }
  } catch (error) {
    console.error('Terjadi kesalahan saat verifikasi lisensi:', error);
    window.location.replace(REDIRECT_URL);
  }
}

// --- Mulai seluruh proses ---
verifyAndStart();